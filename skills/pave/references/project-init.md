...
project knowledge set before feature work begins.
...
4. Treat plugin installation and repo runtime setup as separate steps.
...
5. To apply PAVE runtime files to the repo, run
...
6. Use `scripts/init_repo.js <repo>` as the JavaScript automation
   helper when direct repo initialization is preferred.
7. Fill `AGENTS.md` declared verification commands with real repo
   commands, or mark missing commands as setup gaps.
8. Create or update initial docs:
   - `docs/00-overview.md`
   - `docs/01-roadmap.md`
...
9. Run `scripts/doctor.js <repo> --companions <profile>`. Use
   `scripts/check_companions.sh` only when companion detection needs
   troubleshooting.
10. Report generated files, companion status, unresolved decisions, and
